...
public class LinkerLoader {
  ...
  */
  ...
  public void chargeProgram(ArrayList<int[]> data, Memory memory) throws FileNotFoundException, IOException {    
    PrintWriter outfile = new PrintWriter("programBinary.txt", "UTF-8");
    int addr = 0;    
    for (int[] line : data) {    	
    	System.out.println(Arrays.toString(line));
    	for (int i = 0; i < 5; i++)  memory.set(memory.getOrg() + addr++, line[i]);      
	}    
    outfile.close();
  }
